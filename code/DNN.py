...
df1=pd.read_csv('nn_beta_ch_ee_fl_MPAA_MPAHA.csv',header=None)
df2=pd.read_csv('nn_beta_ch_ee_fl_MPAA_MPAHA_syn_upto80_borderline2.csv',header=None)
...
def multNN1(seed, df1, df2):
    ...
    list1= ind_val
    with open('nn_beta_ch_ee_fl_MPAA_MPAHA_syn_upto80_borderline2-dnn-prediction.csv', 'a') as csvFile:
...
for i in range(0, 100):
    result.append(multNN1(i,df1, df2))


dfResultNormal=pd.DataFrame(result, columns=['seed', 'train_rmse', 'val_rmse', 'test_rmse'])

dfResultNormal.describe()
dfResultNormal.to_csv('nn_beta_ch_ee_fl_MPAA_MPAHA_syn_upto80_borderline2-dnn-out.csv')
