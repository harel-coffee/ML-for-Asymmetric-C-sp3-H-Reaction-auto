...
mydata = np.genfromtxt('beta_ch_ee_mls_MPAA_MPAHA_MPAAM.csv', delimiter=',')
...
mydata_withsynthetic = np.genfromtxt('beta_ch_ee_mls_MPAA_MPAHA_MPAAM_syn_upto80_borderline2.csv', delimiter=',')
...
def compute_testrmse(seed,best_numest):
    ...
    final_regr = DecisionTreeRegressor(criterion='mse', splitter='best', max_depth=final_best_estimator, min_samples_split=2, 
                                     min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features=None, 
                                     random_state=42, max_leaf_nodes=int(math.sqrt(len(final_train_feat[0]))), min_impurity_decrease=0.0, 
                                     min_impurity_split=None, presort=False)
    ...
    with open('beta_ch_ee_mls_MPAA_MPAHA_MPAAM_syn_upto80_borderline2_dt-synthetic-out.csv', 'a') as csvFile:
