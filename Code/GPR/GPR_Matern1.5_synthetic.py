...
def kfoldcv(seed,lengthval):
    ...
    for i in np.arange(len(splitpoints)):
        ...
        kernel = ConstantKernel() * Matern(nu=1.5,length_scale=length_val_arg) + WhiteKernel()  
...
def compute_testrmse(seed,best_length_val):
    ...
    kernel = ConstantKernel() * Matern(nu=1.5,length_scale=final_best_length_val)  + WhiteKernel()
    gpr = GaussianProcessRegressor(kernel=kernel, alpha=1e-10, optimizer='fmin_l_bfgs_b', n_restarts_optimizer=0, normalize_y=False, copy_X_train=True,random_state=42)
    final_regr=gpr
    ...
    with open('beta_ch_ee_mls_MPAA_MPAHA_MPAAM_APAO_syn_upto80_borderline2_synthetic-gpr-matern1.5-length-out.csv', 'a') as csvFile:
